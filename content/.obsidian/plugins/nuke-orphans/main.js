...
var NukeOrphansSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl)
      ...
      .addTextArea((text) =>
        text
          .setPlaceholder(this.app.vault.config.attachmentFolderPath)
          .setValue(this.plugin.settings.attachmentsPaths.join("\n"))
          .onChange((value) =>
            __async(this, null, function* () {
              this.plugin.settings.attachmentsPaths = value
                .split("\n")
                .map((x) => x.trim())
                .filter((x) => x.length > 0);
    ...
    new import_obsidian.Setting(containerEl)
      ...
      .addText((text) =>
        text
          .setPlaceholder(
            this.plugin.shouldUseSystemTrash() ? "system trash" : ".trash/",
          )
      ...
      .addText((text) => {
        function resetColor() {
          text.inputEl.classList.remove(
            CSS_CLASS_CHECK_PASS,
            CSS_CLASS_CHECK_FAIL,
          );
        }
        text.onChange((value) => {
          resetColor();
          ...
          else text.inputEl.classList.add(CSS_CLASS_CHECK_PASS);
        });
      ...
      .addEventListener("click", () =>
        __async(this, null, function* () {
          yield navigator.clipboard.writeText(
            this.files.map((file) => file.path).join("\n"),
          );
      ...
      .addEventListener("click", () =>
        __async(this, null, function* () {
          if (this.trashFolderPath.length > 0) {
            ...
            this.files.forEach((file) =>
              __async(this, null, function* () {
                return yield this.app.fileManager.renameFile(
                  file,
                  path.join(this.trashFolderPath, file.name),
                );
              }),
            );
          } else
          ...
          new import_obsidian2.Notice(
            "Trashed " + this.files.length + " files",
          );
          this.close();
        }),
      );
  ...
  isAttachment(file) {
    return this.getAttachmentsPaths().some((element) => {
      ...
      if (element.startsWith("./") && file.parent.name == element.substring(2))
        return true;
      if (file.parent.path == element) return true;
    });
  }
  getOrphans() {
    const links = new Set(
      Object.values(this.app.metadataCache.resolvedLinks).flatMap((x) =>
        Object.keys(x),
      ),
    );
    ...
    return this.app.vault
      .getFiles()
      .filter((file) => !links.has(file.path) && !filter.test(file.path));
  }
  trash(files) {
    if (files.length > 0)
      new TrashFilesModal(
        this.app,
        files,
        this.settings.trashFolderOverride,
        this.shouldUseSystemTrash(),
      ).open();
    else new import_obsidian3.Notice("No orphaned files have been found");
  }
  onload() {
    return __async(this, null, function* () {
      ...
      this.addCommand({
        ...
        callback: () =>
          this.trash(
            this.getOrphans().filter((file) => this.isAttachment(file)),
          ),
      });
      this.addCommand({
        ...
        callback: () =>
          this.trash(
            this.getOrphans().filter((file) => file.extension === "md"),
          ),
      });
      this.addCommand({
        id: "nuke-orphaned",

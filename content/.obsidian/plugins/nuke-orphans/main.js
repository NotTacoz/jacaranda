...
var NukeOrphansSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl)
      ...
      .addText((text) =>
        text
          .setPlaceholder(this.plugin.shouldUseSystemTrash() ? "system trash" : ".trash/")
...
var NukeOrphansPlugin = class extends import_obsidian3.Plugin {
  ...
  isAttachment(file) {
    return this.getAttachmentsPaths().some((element) => {
      ...
      if (element.startsWith("./") && file.parent.name == element.substring(2)) return true
      if (file.parent.path == element) return true
    })
  }
  getOrphans() {
    const links = new Set(
      Object.values(this.app.metadataCache.resolvedLinks).flatMap((x) => Object.keys(x)),
    )

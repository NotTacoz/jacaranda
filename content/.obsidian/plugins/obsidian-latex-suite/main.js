...
var TreeNode = class {
      ...
      for (
        ...
        if (next instanceof TreeBuffer) {
          ...
          if (
            ...
            next.props &&
            (mounted = next.prop(NodeProp.mounted)) &&
            !mounted.overlay
          )
  ...
  enter(pos, side, mode = 0) {
    ...
    if (
      ...
      (mounted = this._tree.prop(NodeProp.mounted)) &&
      mounted.overlay
    ) {
...
function buildTree(data) {
  ...
  function makeRepeatLeaf(children2, positions2, base, i, from, to, type, lookAhead2) {
    ...
    children2.push(
      makeTree(nodeSet.types[type], localChildren, localPositions, to - from, lookAhead2 - to),
    )
...
var Stack = class {
  ...
  shift(action, next, nextEnd) {
    ...
    if (action & 131072) {
      ...
      this.pos = nextEnd
      this.shiftContext(next, start2)
      if (next <= this.p.parser.maxNode) this.buffer.push(next, start2, nextEnd, 4)
    }
  }
  ...
  get deadEnd() {
    ...
    let { parser: parser2 } = this.p
    return (
      parser2.data[parser2.stateSlot(this.state, 1)] == 65535 && !parser2.stateSlot(this.state, 4)
    )
  }
  restart() {
...
var InputStream = class {
  ...
  acceptToken(token, endOffset = 0) {
    ...
    if (end2 == null || end2 < this.token.start) throw new RangeError("Token end out of bounds")
  ...
  addActions(stack, token, end2, index) {
    ...
    for (let set = 0; set < 2; set++) {
      for (let i = parser2.stateSlot(state, set ? 2 : 1); ; i += 3) {
        ...
        if (data[i] == token) index = this.putAction(pair(data, i + 1), token, end2, index)
      }
    }
    return index
  }
...
var Parse = class {
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (this.stoppedAt != null && start2 > this.stoppedAt) return stack.forceReduce() ? stack : null
    if (this.fragments) {
      ...
      for (let cached = this.fragments.nodeAt(start2); cached; ) {
        ...
        if (
          ...
          if (verbose)
            console.log(
              base + this.stackID(stack) + ` (via reuse of ${parser2.getName(cached.type.id)})`,
            )
          return true
        }
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose)
        console.log(
          base +
            this.stackID(stack) +
            ` (via always-reduce ${parser2.getName(defaultReduce & 65535)})`,
        )
      return true
    }
      ...
      if (verbose)
        console.log(
          base +
            ...
            ` (via ${
              (action & 65536) == 0 ? "shift" : `reduce of ${parser2.getName(action & 65535)}`
...
function findOffset(data, start2, term) {
  for (let i = start2, next; (next = data[i]) != 65535; i++) if (next == term) return i - start2
  return -1
}
...
var jsHighlight = (0, import_highlight.styleTags)({
  ...
  "CallExpression/MemberExpression/PropertyName": import_highlight.tags.function(
  ...
  parser: /* @__PURE__ */ parser.configure({
    props: [
      /* @__PURE__ */ import_language.indentNodeProp.add({
        ...
        "Statement Property": /* @__PURE__ */ (0, import_language.continuedIndent)({
          except: /^{/,
        }),
        JSXElement(context) {
  ...
  (view, from, to, text) => {
    ...
    let changes = state.changeByRange((range) => {
      ...
      if (text == ">" && around.name == "JSXFragmentTag") {
        ...
        if (
          empty.from == head - 1 &&
          ((_c = base.lastChild) === null || _c === void 0 ? void 0 : _c.name) != "JSXEndTag" &&
          (name = elementName(
            state.doc,
...
var obsidianHighlightStyle = import_language2.HighlightStyle.define([
  ...
  {
    tag: [import_highlight2.tags.operator, import_highlight2.tags.operatorKeyword],
...
var obsidian = [obsidianTheme, (0, import_language2.syntaxHighlighting)(obsidianHighlightStyle)]
...
var basicSetup = [
  ...
  (0, import_language3.syntaxHighlighting)(import_language3.defaultHighlightStyle, {
    fallback: true,
  }),
...
function concealModifiedGreekLetters(eqn, greekSymbolMap) {
  ...
  const regexStr =
    "\\\\(underline|boldsymbol){\\\\(" + escapeRegex(greekSymbolNames.join("|")) + ")}"
  const regex = new RegExp(regexStr, "g")
  const matches = [...eqn.matchAll(regex)]
  const concealments = []
  for (const match of matches) {
...
function conceal(view) {
  ...
  const mousedown =
    (_a = view.plugin(import_obsidian2.livePreviewState)) == null ? void 0 : _a.mousedown
    ...
    ;(0, import_language5.syntaxTree)(view.state).iterate({
      ...
      enter: (node) => {
        ...
        for (const concealment of concealments) {
          ...
          if (!mousedown && selectionAndRangeOverlap(selection, start2, end2)) continue
          if (start2 === end2) {
            widgets.push(
              import_view4.Decoration.widget({
...
var concealPlugin = import_view4.ViewPlugin.fromClass(
  class {
    constructor(view) {
...
function highlightCursorBrackets(view) {
  ...
  if (!bounds) return import_view5.Decoration.set(widgets, true)
  const eqn = view.state.doc.sliceString(bounds.start, bounds.end)
  const openBrackets = ["{", "[", "("]
  const brackets2 = ["{", "[", "(", "}", "]", ")"]
  let done = false
...
var colorPairedBracketsPlugin = import_view5.ViewPlugin.fromClass(
  class {
    constructor(view) {
...
var colorPairedBracketsPluginLowestPrec = import_state4.Prec.lowest(
  colorPairedBracketsPlugin.extension,
)
var highlightCursorBracketsPlugin = import_view5.ViewPlugin.fromClass(
  class {
    constructor(view) {
...
var cursorTooltipField = import_state5.StateField.define({
  ...
  update(tooltips2, tr) {
    if (!tr.docChanged && !tr.selection) return tooltips2
  ...
  update(value, tr) {
    ...
    for (const effect4 of tr.effects) {
      if (effect4.is(addMark)) {
        ...
        value = value.update({
          filter: (from, to, ref) => ref.spec[effect4.value.attribute] !== effect4.value.reference,
        })
      }
    }
    return value
  ...
  update(oldState, transaction) {
    ...
    for (const effect4 of transaction.effects) {
      if (effect4.is(addTabstopEffect)) {
        ...
        tabstopReferences = tabstopReferences.filter(
...
function getSnippetsFromString(snippetsStr) {
  ...
  if (!validateSnippets(snippets2)) throw "Invalid snippet format."
  return snippets2
}
...
function onFileChange(plugin, file) {
  return __async(this, null, function* () {
    ...
    if (
      ...
      fileIsInSnippetsFolder(plugin, file)
    ) {
      try {
...
var debouncedSetSnippetsFromFileOrFolder = (0, import_obsidian6.debounce)(
  (plugin, path) =>
...
var LatexSuiteSettingTab = class extends import_obsidian7.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty()
    containerEl
      .createEl("div", { text: "Snippets" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    new import_obsidian7.Setting(containerEl)
      .setName("Enabled")
      .setDesc("Whether snippets are enabled.")
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.snippetsEnabled).onChange((value) =>
    ...
    const snippetsSetting = new import_obsidian7.Setting(containerEl)
      .setName("Snippets")
      .setDesc(
    ...
    const change = import_view8.EditorView.updateListener.of((v) =>
      __async(this, null, function* () {
        if (v.docChanged) {
            ...
            () =>
              __async(this, null, function* () {
                this.snippetsEditor.setState(
                  import_state8.EditorState.create({
          ...
          new ConfirmationModal(
            ...
            () =>
              __async(this, null, function* () {
                ...
                this.snippetsEditor.setState(
                  import_state8.EditorState.create({ doc: value, extensions }),
                )
                updateValidityIndicator(true)
                this.plugin.setSnippets(value)
                this.plugin.settings.snippets = value
    ...
    new import_obsidian7.Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.loadSnippetsFromFile).onChange((value) =>
          __async(this, null, function* () {
    ...
    const snippetsFileLoc = new import_obsidian7.Setting(containerEl)
    ...
    line2.setText(
      "e.g. \\dot{x}^{2} + \\dot{y}^{2} will display as \u1E8B\xB2 + \u1E8F\xB2, and \\sqrt{ 1-\\beta^{2} } will display as \u221A{ 1-\u03B2\xB2 }.",
    )
    ...
    new import_obsidian7.Setting(containerEl)
      ...
      .setDesc(fragment)
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.concealEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    containerEl
      .createEl("div", { text: "Highlight and color brackets" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    new import_obsidian7.Setting(containerEl)
      .setName("Color paired brackets")
      .setDesc("Whether to colorize matching brackets.")
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.colorPairedBracketsEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    new import_obsidian7.Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.highlightCursorBracketsEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    containerEl
      .createEl("div", { text: "Math popup preview" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    ...
    new import_obsidian7.Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.mathPreviewEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    containerEl
      .createEl("div", { text: "Auto-fraction" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    new import_obsidian7.Setting(containerEl)
      .setName("Enabled")
      .setDesc("Whether auto-fraction is enabled.")
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.autofractionEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    containerEl
      .createEl("div", { text: "Matrix shortcuts" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    new import_obsidian7.Setting(containerEl)
      .setName("Enabled")
      .setDesc("Whether matrix shortcuts are enabled.")
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.matrixShortcutsEnabled).onChange((value) =>
          __async(this, null, function* () {
    ...
    containerEl
      .createEl("div", { text: "Auto-enlarge brackets" })
      .addClasses(["setting-item", "setting-item-heading", "setting-item-name"])
    new import_obsidian7.Setting(containerEl)
      .setName("Enabled")
      .setDesc("Whether to automatically enlarge brackets containing e.g. sum, int, frac.")
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.autoEnlargeBrackets).onChange((value) =>
          __async(this, null, function* () {
    ...
    new import_obsidian7.Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.removeSnippetWhitespace).onChange((value) =>
          __async(this, null, function* () {
...
var ConfirmationModal = class extends import_obsidian7.Modal {
  constructor(app, body, buttonCallback, clickCallback) {
    ...
    new import_obsidian7.Setting(this.contentEl)
      .addButton((button) => {
...
function getColorIndex(view) {
  ...
  for (; colorIndex < COLORS.length; colorIndex++) {
    if (!currentTabstopReferences.find((p) => p.getColorIndex() === colorIndex)) break
  }
...
function expandSnippets(view) {
  ...
  const undoKeyPresses = import_state10.ChangeSet.of(keyPresses, originalDocLength).invert(
    originalDoc,
  )
  ...
  for (let i = 0; i < snippets2.length; i++) {
    tabstopsToAdd = tabstopsToAdd.concat(
      getTabstopsFromSnippet(view, newPositions[i], snippets2[i].insert),
    )
  }
...
function insertTabstopReferences(view, tabstops, append = false) {
  const numbers = Array.from(new Set(tabstops.map((tabstop) => tabstop.number)))
    .sort()
    .reverse()
  if (!append) {
...
function removeOnlyTabstop(view) {
  ...
  if (currentTabstopReferences.length === 1) {
    ...
    if (shouldClear) clearAllTabstops(reference2.view)
  }
}
...
function consumeAndGotoNextTabstop(view) {
  ...
  if (newMarkers.length === 0) return consumeAndGotoNextTabstop(view)
  ...
  if (oldCursor.eq(newCursor)) return consumeAndGotoNextTabstop(view)
  return true
}
...
var autoEnlargeBrackets = (view, plugin) => {
  ...
  for (let i = start2; i < end2; i++) {
    ...
    const containsTrigger = plugin.autoEnlargeBracketsTriggers.some((word) =>
...
function boxCurrentEquation(view) {
  ...
  const insideBlockEqn =
    view.state.sliceDoc(start2 - 2, start2) === "$$" && view.state.sliceDoc(end2, end2 + 2) === "$$"
...
var LatexSuitePlugin = class extends import_obsidian8.Plugin {
  ...
  loadSettings() {
    return __async(this, null, function* () {
      ...
      this.matrixShortcutsEnvNames = this.settings.matrixShortcutsEnvNames
        .replace(/\s/g, "")
        .split(",")
      this.autoEnlargeBracketsTriggers = this.settings.autoEnlargeBracketsTriggers
        .replace(/\s/g, "")
        .split(",")

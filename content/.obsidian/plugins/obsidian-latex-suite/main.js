      ...
      var stringify = function stringify2(value, replacer, space2) {
        ...
        function serializeKey(key2) {
          ...
          for (var i2 = firstChar.length; i2 < key2.length; i2++) {
            if (
              !util.isIdContinueChar(String.fromCodePoint(key2.codePointAt(i2)))
            ) {
              return quoteString(key2, true);
            }
          }
...
var Tree = class {
  ...
  iterate(spec) {
    ...
    for (
      let c = this.cursor((spec.mode || 0) | IterMode.IncludeAnonymous);
      ;
...
var Stack = class {
  ...
  get deadEnd() {
    ...
    let { parser: parser2 } = this.p;
    return (
      parser2.data[parser2.stateSlot(this.state, 1)] == 65535 &&
      !parser2.stateSlot(this.state, 4)
    );
  }
  restart() {
        ...
        console.log(
          base +
            ...
            ` (via ${
              (action & 65536) == 0
                ? "shift"
                : `reduce of ${parser2.getName(action & 65535)}`
            } for ${parser2.getName(term)} @ ${start2}${
...
function conceal(view) {
  ...
  const mousedown =
    (_a = view.plugin(import_obsidian2.livePreviewState)) == null
      ? void 0
      : _a.mousedown;
...
var cursorTooltipField = import_state5.StateField.define({
  ...
  provide: (f) =>
    import_view6.showTooltip.computeN([f], (state) => state.field(f)),
});
  ...
  update(value, tr) {
    ...
    for (const effect4 of tr.effects) {
      if (effect4.is(addMark)) {
        ...
        value = value.update({
          filter: (from, to, ref) =>
            ref.spec[effect4.value.attribute] !== effect4.value.reference,
        });
      }
    }
    return value;
...
function validateSnippets(snippets2) {
  ...
  for (const snippet of snippets2) {
    if (
      !(snippet.trigger && snippet.replacement && snippet.options != void 0)
    ) {
      valid = false;
      break;
    }
      ...
      if (fileOrFolder instanceof import_obsidian6.TFile) {
        ...
        try {
          ...
          new import_obsidian6.Notice(
            `Failed to load snippet file ${fileOrFolder.name}`,
          );
        }
      } else {
...
var LatexSuiteSettingTab = class extends import_obsidian7.PluginSettingTab {
  ...
  display() {
    ...
    const validityIndicator = new import_obsidian7.ExtraButtonComponent(
      validity,
    );
    validityIndicator
      .setIcon("checkmark")
    ...
    this.snippetsEditor = createSnippetsEditor(
        ...
        toggle
          ...
          .onChange((value) =>
            __async(this, null, function* () {
              ...
              if (value) {
                this.plugin.enableExtension(
                  colorPairedBracketsPluginLowestPrec,
                );
              } else {
                this.plugin.disableExtension(
                  colorPairedBracketsPluginLowestPrec,
                );
              }
              yield this.plugin.saveSettings();
        ...
        text
          ...
          .onChange((value) =>
            __async(this, null, function* () {
              ...
              this.plugin.matrixShortcutsEnvNames = value
                .replace(/\s/g, "")
                .split(",");
              yield this.plugin.saveSettings();
            }),
        ...
        text
          ...
          .onChange((value) =>
            __async(this, null, function* () {
              ...
              this.plugin.autoEnlargeBracketsTriggers = value
                .replace(/\s/g, "")
                .split(",");
              yield this.plugin.saveSettings();
            }),
...
function expandSnippets(view) {
  ...
  const undoKeyPresses = import_state10.ChangeSet.of(
    keyPresses,
    originalDocLength,
  ).invert(originalDoc);
  const changesAsChangeSet = import_state10.ChangeSet.of(
    changes,
    originalDocLength,
  );
...
var runSnippetCursor = (view, key, withinMath, range, plugin) => {
  ...
  for (const snippet of plugin.snippets) {
    ...
    if (
      snippet.options.contains("A") ||
      snippet.replacement.contains("${VISUAL}")
    ) {
...
var runAutoFractionCursor = (view, range, plugin) => {
  ...
  if (from != to) {
    ...
    for (let i = curLine.length - 1; i >= eqnStart; i--) {
      ...
      if (
        " $([{\n"
          .concat(plugin.settings.autofractionBreakingChars)
          .contains(curChar)
      ) {
        start2 = i + 1;
        break;
...
var LatexSuitePlugin = class extends import_obsidian8.Plugin {
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      this.registerEditorExtension(
        import_view10.EditorView.updateListener.of(this.handleUpdate),
      );
      this.registerEditorExtension(
        (0, import_view10.tooltips)({ position: "absolute" }),
      );
      this.registerEditorExtension(this.editorExtensions);
      this.registerEvent(
        this.app.vault.on("modify", (file) => onFileChange(this, file)),
      );
      this.registerEvent(
        this.app.vault.on("delete", (file) => onFileDelete(this, file)),
      );
      this.registerEvent(
        this.app.vault.on("create", (file) => onFileCreate(this, file)),
      );
      this.addEditorCommands();
    });
  }
  ...
  loadSettings() {
    return __async(this, null, function* () {
      ...
      this.matrixShortcutsEnvNames = this.settings.matrixShortcutsEnvNames
        .replace(/\s/g, "")
        .split(",");
      this.autoEnlargeBracketsTriggers =
        this.settings.autoEnlargeBracketsTriggers.replace(/\s/g, "").split(",");

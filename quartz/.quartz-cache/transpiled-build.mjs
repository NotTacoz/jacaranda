...
function slugifyFilePath(fp, excludeExt) {
  ...
  let ext = _getFileExtension(fp),
    withoutFileExt = fp.replace(new RegExp(ext + "$"), "");
  (excludeExt || [".md", ".html", void 0].includes(ext)) && (ext = "");
  let slug2 = withoutFileExt
    .split("/")
    .map((segment) => segment.replace(/\s/g, "-").replace(/%/g, "-percent"))
    .join("/")
    .replace(/\/$/, "");
  return (
    _endsWith(slug2, "_index") && (slug2 = slug2.replace(/_index$/, "index")),
    slug2 + ext
  );
}
__name(slugifyFilePath, "slugifyFilePath");
function simplifySlug(fp) {
  return _stripSlashes(_trimSuffix(fp, "index"), !0);
}
__name(simplifySlug, "simplifySlug");
function transformInternalLink(link) {
  let [fplike, anchor] = splitAnchor(decodeURI(link)),
    folderPath = _isFolderPath(fplike),
    segments = fplike.split("/").filter((x) => x.length > 0),
    prefix = segments.filter(_isRelativeSegment).join("/"),
    fp = segments
      .filter((seg) => !_isRelativeSegment(seg) && seg !== "")
      .join("/"),
    simpleSlug = simplifySlug(slugifyFilePath(fp)),
    joined = joinSegments(_stripSlashes(prefix), _stripSlashes(simpleSlug)),
    trail = folderPath ? "/" : "";
  return _addRelativeToStart(joined) + trail + anchor;
}
...
function pathToRoot(slug2) {
  let rootPath = slug2
    ...
    .map((_) => "..")
    .join("/");
  return rootPath.length === 0 && (rootPath = "."), rootPath;
}
...
function splitAnchor(link) {
  ...
  return (
    (anchor = anchor === void 0 ? "" : "#" + slugAnchor(anchor)), [fp, anchor]
  );
}
...
function transformLink(src, target, opts) {
  ...
  if (opts.strategy === "relative") return targetSlug;
  {
    let folderTail = _isFolderPath(targetSlug) ? "/" : "",
      canonicalSlug = _stripSlashes(targetSlug.slice(1)),
      [targetCanonical, targetAnchor] = splitAnchor(canonicalSlug);
    if (opts.strategy === "shortest") {
      let matchingFileNames = opts.allSlugs.filter((slug2) => {
        let fileName = slug2.split("/").at(-1);
        return targetCanonical === fileName;
      });
      if (matchingFileNames.length === 1) {
        let targetSlug2 = matchingFileNames[0];
...
function _trimSuffix(s, suffix) {
  return _endsWith(s, suffix) && (s = s.slice(0, -suffix.length)), s;
}
...
function _stripSlashes(s, onlyStripPrefix) {
  return (
    ...
    !onlyStripPrefix && s.endsWith("/") && (s = s.slice(0, -1)),
    s
  );
}
...
function _addRelativeToStart(s) {
  return (
    s === "" && (s = "."), s.startsWith(".") || (s = joinSegments(".", s)), s
  );
}
...
var QuartzLogger = class {
  static {
    __name(this, "QuartzLogger");
  }
  verbose;
  spinner;
  constructor(verbose) {
  ...
  start(text) {
    ...
    this.verbose || this.spinner.stop(!0), text && console.log(text);
  }
};
...
` +
    ...
    return;
  let reachedEndOfLegibleTrace = !1;
...
`,
    ...
    .slice(1)) {
    if (reachedEndOfLegibleTrace) break;
    line.includes("node_modules") ||
      (lines.push(` ${line}`),
      rootFile.test(line) && (reachedEndOfLegibleTrace = !0));
  }
  let traceMsg = lines.join(`
`);
  if (isMainThread) console.error(traceMsg), process2.exit(1);
  else throw new Error(traceMsg);
}
__name(trace, "trace");
function createProcessor(ctx) {
  let transformers = ctx.cfg.plugins.transformers,
    processor = unified().use(remarkParse);
  for (let plugin of transformers.filter((p) => p.markdownPlugins))
    processor = processor.use(plugin.markdownPlugins(ctx));
  processor = processor.use(remarkRehype, { allowDangerousHtml: !0 });
  for (let plugin of transformers.filter((p) => p.htmlPlugins))
...
async function transpileWorkerScript() {
  let cacheFile = "./.quartz-cache/transpiled-worker.mjs",
    fp = "./quartz/worker.ts";
...
__name(transpileWorkerScript, "transpileWorkerScript");
function createFileParser(ctx, fps) {
  ...
  return async (processor) => {
    ...
    for (let fp of fps)
      try {
        let perf = new PerfTimer(),
          file = await read(fp);
        file.value = file.value.toString().trim();
        for (let plugin of cfg.plugins.transformers.filter(
          (p) => p.textTransform,
        ))
          file.value = plugin.textTransform(ctx, file.value);
        (file.data.slug = slugifyFilePath(
          path.posix.relative(argv.directory, file.path),
        )),
          (file.data.filePath = fp);
        let ast = processor.parse(file),
          newAst = await processor.run(ast, file);
...
var clamp = __name(
  (num, min, max) => Math.min(Math.max(Math.round(num), min), max),
  "clamp",
);
async function parseMarkdown(ctx, fps) {
  let { argv } = ctx,
    perf = new PerfTimer(),
    log = new QuartzLogger(argv.verbose),
    CHUNK_SIZE = 128,
    concurrency = ctx.argv.concurrency ?? clamp(fps.length / CHUNK_SIZE, 1, 4),
    res = [];
  if (
    ...
    try {
      let processor = createProcessor(ctx);
      res = await createFileParser(ctx, fps)(processor);
    } catch (error) {
      throw (log.end(), error);
    }
  else {
    await transpileWorkerScript();
    let pool = workerpool.pool("./quartz/bootstrap-worker.mjs", {
        minWorkers: "max",
        maxWorkers: concurrency,
        workerType: "thread",
      }),
      childPromises = [];
    for (let chunk of chunks(fps, CHUNK_SIZE))
      childPromises.push(pool.exec("parseFiles", [argv, chunk, ctx.allSlugs]));
    (res = (
      await WorkerPromise.all(childPromises).catch((err) => {
        let errString = err.toString().slice(6);
        console.error(errString), process.exit(1);
      })
    ).flat()),
      await pool.terminate();
  }
  return (
    log.end(`Parsed ${res.length} Markdown files in ${perf.timeSince()}`), res
  );
}
__name(parseMarkdown, "parseMarkdown");
function filterContent(ctx, content) {
  let { cfg, argv } = ctx,
    perf = new PerfTimer(),
    initialLength = content.length;
  for (let plugin of cfg.plugins.filters) {
    let updatedContent = content.filter((item) =>
      plugin.shouldPublish(ctx, item),
    );
    if (argv.verbose) {
      let diff = content.filter((x) => !updatedContent.includes(x));
...
var defaultOptions = { delims: "---", language: "yaml" },
  FrontMatter = __name((userOpts) => {
...
var defaultOptions2 = { enableSmartyPants: !0, linkHeadings: !0 },
  GitHubFlavoredMarkdown = __name((userOpts) => {
    let opts = { ...defaultOptions2, ...userOpts };
    return {
...
var defaultOptions3 = { priority: ["frontmatter", "git", "filesystem"] },
  CreatedModifiedDate = __name((userOpts) => {
    let opts = { ...defaultOptions3, ...userOpts };
    return {
          ...
          () => {
            ...
            return async (_tree, file) => {
              let created,
                modified,
                published,
                fp = path2.posix.join(file.cwd, file.data.filePath);
              for (let source of opts.priority)
                if (source === "filesystem") {
                  let st = await fs.promises.stat(fp);
                  (created ||= st.birthtimeMs), (modified ||= st.mtimeMs);
                } else
                  source === "frontmatter" && file.data.frontmatter
                    ...
                    : source === "git" &&
                      (repo || (repo = new Repository(file.cwd)),
...
var Latex = __name((opts) => {
  let engine = opts?.renderEngine ?? "katex";
  return {
    name: "Latex",
    markdownPlugins() {
      return [remarkMath];
...
var defaultOptions4 = { descriptionLength: 150 },
  escapeHTML = __name(
    (unsafe) =>
      unsafe
        .replaceAll("&", "&amp;")
  ...
  Description = __name((userOpts) => {
    let opts = { ...defaultOptions4, ...userOpts };
    return {
      name: "Description",
      htmlPlugins() {
        return [
          () => async (tree, file) => {
            let frontMatterDescription = file.data.frontmatter?.description,
              ...
              sentences = (frontMatterDescription ?? text)
                .replace(/\s+/g, " ")
                .split("."),
              finalDesc = "",
              sentenceIdx = 0,
              len = opts.descriptionLength;
            for (; finalDesc.length < len; ) {
              let sentence = sentences[sentenceIdx];
              if (!sentence) break;
              (finalDesc += sentence + "."), sentenceIdx++;
            }
            (file.data.description = finalDesc), (file.data.text = text);
          },
        ];
      },
    };
...
var defaultOptions5 = { markdownLinkResolution: "absolute", prettyLinks: !0 },
  CrawlLinks = __name((userOpts) => {
    let opts = { ...defaultOptions5, ...userOpts };
    return {
      name: "LinkProcessing",
      htmlPlugins(ctx) {
        return [
          () => (tree, file) => {
            let curSlug = simplifySlug(file.data.slug),
              outgoing = new Set(),
              transformOptions = {
            ...
            visit(tree, "element", (node, _index, _parent) => {
              if (
                ...
                if (
                  ...
                  let canonicalDest = new URL(
                      dest,
                      `https://base.com/${curSlug}`,
                    ).pathname,
                    [destCanonical, _destAnchor] = splitAnchor(canonicalDest),
                    simple = decodeURIComponent(simplifySlug(destCanonical));
                  outgoing.add(simple);
                }
                opts.prettyLinks &&
                  ...
                  (node.children[0].value = path3.basename(
...
var defaultOptions6 = {
  ...
  icons = {
    infoIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>',
    pencilIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="2" x2="22" y2="6"></line><path d="M7.5 20.5 19 9l-4-4L3.5 16.5 2 22z"></path></svg>',
    clipboardListIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="2" width="8" height="4" rx="1" ry="1"></rect><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"></path><path d="M12 11h4"></path><path d="M12 16h4"></path><path d="M8 11h.01"></path><path d="M8 16h.01"></path></svg>',
    checkCircleIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"></path><path d="m9 12 2 2 4-4"></path></svg>',
    flameIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"></path></svg>',
    checkIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',
    helpCircleIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>',
    alertTriangleIcon:
    ...
    zapIcon:
      '<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>',
    bugIcon:
...
var capitalize = __name(
    (s) => s.substring(0, 1).toUpperCase() + s.substring(1),
    "capitalize",
  ),
  wikilinkRegex = new RegExp(
    /!?\[\[([^\[\]\|\#]+)?(#[^\[\]\|\#]+)?(\|[^\[\]\|\#]+)?\]\]/,
    "g",
  ),
  highlightRegex = new RegExp(/==(.+)==/, "g"),
  commentRegex = new RegExp(/%%(.+)%%/, "g"),
  calloutRegex = new RegExp(/^\[\!(\w+)\]([+-]?)/),
  calloutLineRegex = new RegExp(/^> *\[\!\w+\][+-]?.*$/, "gm"),
  tagRegex = new RegExp(/(?:^| )#(\p{L}+)/, "gu"),
  ObsidianFlavoredMarkdown = __name((userOpts) => {
    let opts = { ...defaultOptions6, ...userOpts },
      mdastToHtml = __name((ast) => {
        let hast = toHast(ast, { allowDangerousHtml: !0 });
        return toHtml(hast, { allowDangerousHtml: !0 });
      }, "mdastToHtml"),
              ...
              visit2(tree, "html", (node) => {
                typeof replace == "string"
                  ...
                  : (node.value = node.value.replaceAll(
                      regex,
                      (substring, ...args) => {
                        let replaceValue = replace(substring, ...args);
                        return typeof replaceValue == "string"
                          ? replaceValue
                          : Array.isArray(replaceValue)
                          ? replaceValue.map(mdastToHtml).join("")
                          : typeof replaceValue == "object" &&
                            replaceValue !== null
                          ? mdastToHtml(replaceValue)
                          : substring;
                      },
                    ));
              }),
          ...
          opts.wikilinks &&
            ...
            (src = src.replaceAll(wikilinkRegex, (value, ...capture) => {
              let [rawFp, rawHeader, rawAlias] = capture,
                fp = rawFp ?? "",
                anchor = rawHeader?.trim().slice(1),
                displayAnchor = anchor ? `#${slugAnchor2(anchor)}` : "",
                displayAlias = rawAlias ?? rawHeader?.replace("#", "|") ?? "";
              return `${
                value.startsWith("!") ? "!" : ""
              }[[${fp}${displayAnchor}${displayAlias}]]`;
            }))),
          src
        );
      },
          ...
          opts.wikilinks &&
            plugins.push(() => (tree, _file) => {
              findAndReplace(tree, wikilinkRegex, (value, ...capture) => {
                ...
                if (value.startsWith("!")) {
                  let ext = path4.extname(fp).toLowerCase(),
                    url2 = slugifyFilePath(fp);
                  if (
            ...
            plugins.push(() => (tree, _file) => {
              visit2(tree, "blockquote", (node) => {
                ...
                if (
                  ...
                  return;
                let text = firstChild.children[0].value,
                  restChildren = firstChild.children.slice(1),
                  [firstLine, ...remainingLines] = text.split(`
`),
                  remainingText = remainingLines.join(`
`),
                  match = firstLine.match(calloutRegex);
                if (match && match.input) {
                  let [calloutDirective, typeString, collapseChar] = match,
                    calloutType = canonicalizeCallout(typeString.toLowerCase()),
                      ...
                      children: [
                        {
                          ...
                          value:
                            (match.input
                              ...
                              .trim() || capitalize(calloutType)) + " ",
                        },
                        ...restChildren,
                      ],
                    },
                    title = mdastToHtml(titleNode),
                    toggleIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="fold">
          ...
          opts.parseTags &&
            plugins.push(() => (tree, file) => {
              let base = pathToRoot(file.data.slug);
              findAndReplace(
                tree,
                tagRegex,
                (_value, tag) => (
...
var relrefRegex = new RegExp(
    /\[([^\]]+)\]\(\{\{< relref "([^"]+)" >\}\}\)/,
    "g",
  ),
  predefinedHeadingIdRegex = new RegExp(/(.*) {#(?:.*)}/, "g"),
  hugoShortcodeRegex = new RegExp(/{{(.*)}}/, "g"),
  figureTagRegex = new RegExp(/< ?figure src="(.*)" ?>/, "g");
import rehypePrettyCode from "rehype-pretty-code";
var SyntaxHighlighting = __name(
  () => ({
    name: "SyntaxHighlighting",
    htmlPlugins() {
      return [[rehypePrettyCode, { theme: "css-variables" }]];
    },
  }),
  "SyntaxHighlighting",
);
...
var defaultOptions7 = { maxDepth: 3, minEntries: 1, showByDefault: !0 },
  TableOfContents = __name((userOpts) => {
    let opts = { ...defaultOptions7, ...userOpts };
    return {
      name: "TableOfContents",
      markdownPlugins() {
        return [
          () => async (tree, file) => {
            if (file.data.frontmatter?.enableToc ?? opts.showByDefault) {
              let slugAnchor3 = new Slugger(),
                toc = [],
                highestDepth = opts.maxDepth;
...
var RemoveDrafts = __name(
  () => ({
    name: "RemoveDrafts",
    shouldPublish(_ctx, [_tree, vfile]) {
      return !(vfile.data?.frontmatter?.draft ?? !1);
    },
  }),
  "RemoveDrafts",
);
...
function Header({ children }) {
  return children.length > 0 ? jsx("header", { children }) : null;
}
...
`;
var Header_default = __name(() => Header, "default");
var clipboard_inline_default = `// quartz/components/scripts/quartz/components/scripts/clipboard.inline.ts
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
import { jsx as jsx2 } from "preact/jsx-runtime";
function Body({ children }) {
  return jsx2("div", { id: "quartz-body", children });
}
__name(Body, "Body");
Body.afterDOMLoaded = clipboard_inline_default;
Body.css = clipboard_default;
var Body_default = __name(() => Body, "default");
import { render } from "preact-render-to-string";
import { randomUUID } from "crypto";
import { jsx as jsx3 } from "preact/jsx-runtime";
function JSResourceToScriptElement(resource, preserve) {
  let scriptType = resource.moduleType ?? "application/javascript",
    spaPreserve = preserve ?? resource.spaPreserve;
  if (resource.contentType === "external")
    return jsx3(
      "script",
      { src: resource.src, type: scriptType, "spa-preserve": spaPreserve },
      resource.src,
    );
  {
    let content = resource.script;
    return jsx3(
      "script",
      { type: scriptType, "spa-preserve": spaPreserve, children: content },
      randomUUID(),
    );
  }
}
...
function pageResources(slug2, staticResources) {
  let baseDir = pathToRoot(slug2),
    contentIndexScript = `const fetchData = fetch(\`${joinSegments(
      baseDir,
...
function renderPage(slug2, componentData, components, pageResources2) {
  let {
    ...
    Body2 = Body_default(),
    LeftComponent = jsx4("div", {
      class: "left sidebar",
      children: left.map((BodyComponent) =>
        jsx4(BodyComponent, { ...componentData }),
      ),
    }),
    RightComponent = jsx4("div", {
      class: "right sidebar",
      children: right.map((BodyComponent) =>
        jsx4(BodyComponent, { ...componentData }),
      ),
    }),
    doc = jsxs("html", {
      children: [
        jsx4(Head, { ...componentData }),
        jsx4("body", {
          "data-slug": slug2,
          children: jsxs("div", {
            id: "quartz-root",
                    ...
                    children: [
                      jsxs("div", {
                        ...
                        children: [
                          jsx4(Header2, {
                            ...componentData,
                            children: header.map((HeaderComponent) =>
                              jsx4(HeaderComponent, { ...componentData }),
                            ),
                          }),
                          jsx4("div", {
                            class: "popover-hint",
                            children: beforeBody.map((BodyComponent) =>
                              jsx4(BodyComponent, { ...componentData }),
                            ),
                          }),
                        ],
                      }),
                      jsx4(Content2, { ...componentData }),
                    ],
                  }),
                  RightComponent,
                ],
              }),
              jsx4(Footer, { ...componentData }),
            ],
          }),
        }),
        pageResources2.js
...
function ArticleTitle({ fileData }) {
  ...
  return title ? jsx5("h1", { class: "article-title", children: title }) : null;
}
...
`;
var ArticleTitle_default = __name(() => ArticleTitle, "default");
import { Fragment, jsx as jsx6, jsxs as jsxs2 } from "preact/jsx-runtime";
import { toJsxRuntime } from "hast-util-to-jsx-runtime";
import { jsx as jsx7 } from "preact/jsx-runtime";
function Content({ tree }) {
  let content = toJsxRuntime(tree, {
    Fragment,
    jsx: jsx6,
    jsxs: jsxs2,
    elementAttributeNameCase: "html",
  });
  return jsx7("article", { class: "popover-hint", children: content });
}
__name(Content, "Content");
var Content_default = __name(() => Content, "default");
import {
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
...
function Date2({ date }) {
  return jsx8(Fragment2, { children: formatDate(date) });
}
__name(Date2, "Date");
...
function byDateAndAlphabetical(cfg) {
  return (f1, f2) => {
    ...
    if (!f1.dates && f2.dates) return 1;
    let f1Title = f1.frontmatter?.title.toLowerCase() ?? "",
      f2Title = f2.frontmatter?.title.toLowerCase() ?? "";
          ...
          children: jsxs3("div", {
            ...
            children: [
              page.dates &&
                jsx9("p", {
                  class: "meta",
                  children: jsx9(Date2, { date: getDate(cfg, page) }),
                }),
              jsx9("div", {
                class: "desc",
                children: jsx9("h3", {
...
function TagContent(props) {
  let { tree, fileData, allFiles } = props,
    slug2 = fileData.slug;
  if (!(slug2?.startsWith("tags/") || slug2 === "tags"))
    throw new Error(
      `Component "TagContent" tried to render a non-tag page: ${slug2}`,
    );
  let tag = simplifySlug(slug2.slice(5)),
    allPagesWithTag = __name(
      (tag2) =>
  ...
  if (tag === "") {
    let tags = [
      ...
      tagItemMap = new Map();
    for (let tag2 of tags) tagItemMap.set(tag2, allPagesWithTag(tag2));
    return jsxs5("div", {
      class: "popover-hint",
      children: [
        jsx11("article", { children: jsx11("p", { children: content }) }),
        jsxs5("p", { children: ["Found ", tags.length, " total tags."] }),
        jsx11("div", {
          children: tags.map((tag2) => {
            let pages = tagItemMap.get(tag2),
              listProps = { ...props, allFiles: pages },
              content2 = allFiles.filter(
                (file) => file.slug === `tags/${tag2}`,
              )[0]?.description;
            return jsxs5("div", {
              children: [
                jsx11("h2", {
                ...
                content2 && jsx11("p", { children: content2 }),
                jsxs5("p", {
                  children: [
                    pages.length,
                    " items with this tag.",
                ...
                jsx11(PageList, { limit: numPages, ...listProps }),
              ],
            });
          }),
        }),
    ...
    return jsxs5("div", {
      class: "popover-hint",
      children: [
        jsx11("article", { children: content }),
        jsxs5("p", { children: [pages.length, " items with this tag."] }),
        jsx11("div", { children: jsx11(PageList, { ...listProps }) }),
      ],
    });
  }
}
...
var TagContent_default = __name(() => TagContent, "default");
import {
...
function FolderContent(props) {
  let { tree, fileData, allFiles } = props,
    folderSlug = _stripSlashes(simplifySlug(fileData.slug)),
    allPagesInFolder = allFiles.filter((file) => {
      let fileSlug = _stripSlashes(simplifySlug(file.slug)),
        prefixed = fileSlug.startsWith(folderSlug) && fileSlug !== folderSlug,
        folderParts = folderSlug.split(path5.posix.sep),
        isDirectChild =
          fileSlug.split(path5.posix.sep).length === folderParts.length + 1;
      return prefixed && isDirectChild;
    }),
    listProps = { ...props, allFiles: allPagesInFolder },
    content =
      tree.children.length === 0
        ? fileData.description
        : toJsxRuntime3(tree, {
  ...
  return jsxs7("div", {
    class: "popover-hint",
    children: [
      jsx13("article", { children: jsx13("p", { children: content }) }),
      jsxs7("p", {
        children: [allPagesInFolder.length, " items under this folder."],
      }),
      jsx13("div", { children: jsx13(PageList, { ...listProps }) }),
    ],
  });
}
__name(FolderContent, "FolderContent");
FolderContent.css = listPage_default + PageList.css;
var FolderContent_default = __name(() => FolderContent, "default");
var darkmode_inline_default = `// quartz/components/scripts/quartz/components/scripts/darkmode.inline.ts
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
    ...
    children: [
      ...
      jsx14("label", {
        ...
        children: jsxs8("svg", {
          ...
          xmlSpace: "preserve",
          children: [
            jsx14("title", { children: "Light mode" }),
            jsx14("path", {
      ...
      jsx14("label", {
        ...
        children: jsxs8("svg", {
          ...
          xmlSpace: "preserve",
          children: [
            jsx14("title", { children: "Dark mode" }),
            jsx14("path", {
...
var Darkmode_default = __name(() => Darkmode, "default");
import { jsx as jsx15, jsxs as jsxs9 } from "preact/jsx-runtime";
var Head_default = __name(() => {
  function Head({ cfg, fileData, externalResources }) {
    let title = fileData.frontmatter?.title ?? "Untitled",
      description = fileData.description?.trim() ?? "No description provided",
      { css, js } = externalResources,
      baseDir = pathToRoot(fileData.slug),
      iconPath = joinSegments(baseDir, "static/icon.png"),
      ogImagePath = `https://${cfg.baseUrl}/static/og-image.png`;
    return jsxs9("head", {
      children: [
        jsx15("title", { children: title }),
        jsx15("meta", { charSet: "utf-8" }),
        jsx15("meta", {
          name: "viewport",
          content: "width=device-width, initial-scale=1.0",
        }),
        jsx15("meta", { property: "og:title", content: title }),
        jsx15("meta", { property: "og:description", content: description }),
        cfg.baseUrl &&
          jsx15("meta", { property: "og:image", content: ogImagePath }),
        jsx15("meta", { property: "og:width", content: "1200" }),
        jsx15("meta", { property: "og:height", content: "675" }),
        jsx15("link", { rel: "icon", href: iconPath }),
        jsx15("meta", { name: "description", content: description }),
        jsx15("meta", { name: "generator", content: "Quartz" }),
        jsx15("link", {
          rel: "preconnect",
          href: "https://fonts.googleapis.com",
        }),
        jsx15("link", { rel: "preconnect", href: "https://fonts.gstatic.com" }),
        css.map((href) =>
          jsx15(
            "link",
            { href, rel: "stylesheet", type: "text/css", "spa-preserve": !0 },
            href,
          ),
        ),
        js
          .filter((resource) => resource.loadTime === "beforeDOMReady")
          .map((res) => JSResourceToScriptElement(res, !0)),
      ],
    });
  }
  return __name(Head, "Head"), Head;
}, "default");
import { jsx as jsx16 } from "preact/jsx-runtime";
function PageTitle({ fileData, cfg }) {
  let title = cfg?.pageTitle ?? "Untitled Quartz",
    baseDir = pathToRoot(fileData.slug);
  return jsx16("h1", {
    class: "page-title",
    children: jsx16("a", { href: baseDir, children: title }),
  });
}
...
`;
var PageTitle_default = __name(() => PageTitle, "default");
import readingTime from "reading-time";
import { jsx as jsx17 } from "preact/jsx-runtime";
var ContentMeta_default = __name(() => {
  function ContentMetadata({ cfg, fileData }) {
    let text = fileData.text;
    if (text) {
      let segments = [],
        { text: timeTaken, words: _words } = readingTime(text);
      return (
        fileData.dates && segments.push(formatDate(getDate(cfg, fileData))),
        segments.push(timeTaken),
        jsx17("p", { class: "content-meta", children: segments.join(", ") })
      );
    } else return null;
  }
  return (
...
function Spacer({ displayClass }) {
  let className = displayClass ? `spacer ${displayClass}` : "spacer";
  return jsx18("div", { class: className });
}
__name(Spacer, "Spacer");
var Spacer_default = __name(() => Spacer, "default");
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
var toc_default = `
button#toc {
  background-color: transparent;
  border: none;
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
...
var defaultOptions8 = { layout: "modern" };
function TableOfContents2({ fileData, displayClass }) {
  return fileData.toc
    ? jsxs10("div", {
        class: `toc ${displayClass}`,
        children: [
          jsxs10("button", {
            type: "button",
            id: "toc",
            children: [
              jsx19("h3", { children: "Table of Contents" }),
              jsx19("svg", {
                xmlns: "http://www.w3.org/2000/svg",
                width: "24",
                height: "24",
                ...
                class: "fold",
                children: jsx19("polyline", { points: "6 9 12 15 18 9" }),
              }),
            ],
          }),
          jsx19("div", {
...
function LegacyTableOfContents({ fileData }) {
  return fileData.toc
    ? jsxs10("details", {
        ...
        children: [
          jsx19("summary", {
            children: jsx19("h3", { children: "Table of Contents" }),
          }),
          jsx19("ul", {
            children: fileData.toc.map((tocEntry) =>
              jsx19(
...
var TableOfContents_default = __name(
  (opts) =>
    (opts?.layout ?? defaultOptions8.layout) === "modern"
      ? TableOfContents2
...
function TagList({ fileData }) {
  ...
  return tags && tags.length > 0
    ? jsx20("ul", {
        ...
        children: tags.map((tag) => {
          let display = `#${tag}`,
            linkDest = baseDir + `/tags/${slugTag(tag)}`;
          return jsx20("li", {
            children: jsx20("a", {
              href: linkDest,
              class: "internal tag-link",
...
`;
var TagList_default = __name(() => TagList, "default");
var graph_inline_default = `// node_modules/d3-dispatch/src/dispatch.js
var noop = { value: () => {
} };
function dispatch() {
...
var defaultOptions9 = {
  ...
  Graph_default = __name((opts) => {
    function Graph() {
      let localGraph = { ...defaultOptions9.localGraph, ...opts?.localGraph },
        globalGraph = { ...defaultOptions9.globalGraph, ...opts?.globalGraph };
      return jsxs11("div", {
        class: "graph",
        children: [
          jsx21("h3", { children: "Graph View" }),
          jsxs11("div", {
            class: "graph-outer",
            children: [
              jsx21("div", {
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
import { jsx as jsx22, jsxs as jsxs12 } from "preact/jsx-runtime";
function Backlinks({ fileData, allFiles }) {
  let slug2 = simplifySlug(fileData.slug),
    backlinkFiles = allFiles.filter((file) => file.links?.includes(slug2));
  return jsxs12("div", {
    class: "backlinks",
    children: [
      jsx22("h3", { children: "Backlinks" }),
      jsx22("ul", {
        class: "overflow",
        children:
          backlinkFiles.length > 0
            ...
            : jsx22("li", { children: "No backlinks found" }),
      }),
    ],
  });
}
...
var Backlinks_default = __name(() => Backlinks, "default");
var search_default = `
.search {
        ...
        jsxs13("div", {
          ...
          children: [
            ...
            jsxs13("svg", {
              ...
              children: [
                jsx23("title", { id: "title", children: "Search" }),
                jsx23("desc", { id: "desc", children: "Search" }),
                jsxs13("g", {
                  class: "search-path",
                  fill: "none",
                  children: [
        ...
        jsx23("div", {
          ...
          children: jsxs13("div", {
            ...
            children: [
              ...
              jsx23("div", { id: "results-container" }),
            ],
          }),
        }),
      ],
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
var version = "4.0.10";
import { jsx as jsx24, jsxs as jsxs14 } from "preact/jsx-runtime";
var Footer_default = __name((opts) => {
  function Footer() {
    let year = new Date().getFullYear(),
      links = opts?.links ?? [];
    return jsxs14("footer", {
      children: [
        jsx24("hr", {}),
        jsxs14("p", {
          children: [
        ...
        jsx24("ul", {
          children: Object.entries(links).map(([text, link]) =>
            jsx24("li", {
              children: jsx24("a", { href: link, children: text }),
            }),
          ),
        }),
      ],
  ...
  return __name(Footer, "Footer"), (Footer.css = footer_default), Footer;
}, "default");
import { Fragment as Fragment5, jsx as jsx25 } from "preact/jsx-runtime";
var DesktopOnly_default = __name((component) => {
  if (component) {
    let DesktopOnly2 = function (props) {
      return jsx25(Component, { displayClass: "desktop-only", ...props });
    };
  ...
  } else return () => jsx25(Fragment5, {});
}, "default");
import { Fragment as Fragment6, jsx as jsx26 } from "preact/jsx-runtime";
var MobileOnly_default = __name((component) => {
  if (component) {
    let MobileOnly2 = function (props) {
      return jsx26(Component, { displayClass: "mobile-only", ...props });
    };
...
var ContentPage = __name((userOpts) => {
  let opts = {
  ...
  return {
    ...
    async emit(ctx, content, resources, emit) {
      let cfg = ctx.cfg.configuration,
        fps = [],
        allFiles = content.map((c) => c[1].data);
      for (let [tree, file] of content) {
        let slug2 = file.data.slug,
          externalResources = pageResources(slug2, resources),
          componentData = {
            fileData: file.data,
            externalResources,
            cfg,
          ...
          content2 = renderPage(slug2, componentData, opts, externalResources),
          fp = await emit({ content: content2, slug: slug2, ext: ".html" });
        fps.push(fp);
      }
      return fps;
    },
...
function defaultProcessedContent(vfileData) {
  let root = { type: "root", children: [] },
    vfile = new VFile("");
  return (vfile.data = vfileData), [root, vfile];
}
__name(defaultProcessedContent, "defaultProcessedContent");
var TagPage = __name((userOpts) => {
  let opts = {
      ...sharedPageComponents,
      ...defaultListPageLayout,
  ...
  return {
    ...
    async emit(ctx, content, resources, emit) {
      ...
      tags.add("index");
      let tagDescriptions = Object.fromEntries(
        [...tags].map((tag) => {
          let title = tag === "" ? "Tag Index" : `Tag: #${tag}`;
          return [
            tag,
      ...
      for (let [tree, file] of content) {
        ...
        if (slug2.startsWith("tags/")) {
          let tag = slug2.slice(5);
          tags.has(tag) && (tagDescriptions[tag] = [tree, file]);
        }
      }
      for (let tag of tags) {
        let slug2 = joinSegments("tags", tag),
          externalResources = pageResources(slug2, resources),
          [tree, file] = tagDescriptions[tag],
          componentData = {
            fileData: file.data,
            externalResources,
            cfg,
...
var FolderPage = __name((userOpts) => {
  let opts = {
      ...sharedPageComponents,
      ...defaultListPageLayout,
  ...
  return {
    ...
    async emit(ctx, content, resources, emit) {
      let fps = [],
        ...
        folders = new Set(
          allFiles.flatMap((data) => {
            let slug2 = data.slug,
              folderName = path6.dirname(slug2 ?? "");
            return slug2 && folderName !== "." && folderName !== "tags"
              ? [folderName]
              : [];
          }),
        ),
      ...
      for (let [tree, file] of content) {
        let slug2 = _stripSlashes(simplifySlug(file.data.slug));
        folders.has(slug2) && (folderDescriptions[slug2] = [tree, file]);
      }
      for (let folder of folders) {
        let slug2 = joinSegments(folder, "index"),
          externalResources = pageResources(slug2, resources),
          [tree, file] = folderDescriptions[folder],
          componentData = {
            fileData: file.data,
            externalResources,
            cfg,
...
function generateSiteMap(cfg, idx) {
  ...
  return `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">${Array.from(
    idx,
  )
...
var ContentIndex = __name(
  (opts) => (
    (opts = { ...defaultOptions10, ...opts }),
    {
      name: "ContentIndex",
      async emit(ctx, content, _resources, emit) {
        ...
        let fp = path7.join("static", "contentIndex"),
          simplifiedIndex = Object.fromEntries(
            Array.from(linkIndex).map(
              ([slug2, content2]) => (
...
var AliasRedirects = __name(
  () => ({
    ...
    async emit({ argv }, content, _resources, emit) {
      let fps = [];
      for (let [_tree, file] of content) {
        let ogSlug = simplifySlug(file.data.slug),
          dir = path8.posix.relative(
            argv.directory,
            file.dirname ?? argv.directory,
          ),
          aliases =
            file.data.frontmatter?.aliases ??
            file.data.frontmatter?.alias ??
            [];
        typeof aliases == "string" && (aliases = [aliases]);
        for (let alias of aliases) {
          let slug2 = path8.posix.join(dir, alias),
            redirUrl = resolveRelative(slug2, file.data.slug),
            fp = await emit({
              content: `
            <!DOCTYPE html>
            <html lang="en-us">
...
async function glob(pattern, cwd, ignorePatterns) {
  return (
    await globby(pattern, { cwd, ignore: ignorePatterns, gitignore: !0 })
  ).map(toPosixPath);
}
__name(glob, "glob");
var Assets = __name(
  () => ({
    ...
    async emit({ argv, cfg }, _content, _resources, _emit) {
      let assetsPath = argv.output,
        fps = await glob("**", argv.directory, [
          "**/*.md",
          ...cfg.configuration.ignorePatterns,
        ]),
        res = [];
      for (let fp of fps) {
        let ext = path10.extname(fp),
          src = joinSegments(argv.directory, fp),
          name = slugifyFilePath(fp, !0) + ext,
          dest = joinSegments(assetsPath, name),
          dir = path10.dirname(dest);
        await fs2.promises.mkdir(dir, { recursive: !0 }),
...
var Static = __name(
  () => ({
    ...
    async emit({ argv, cfg }, _content, _resources, _emit) {
      let staticPath = joinSegments(QUARTZ, "static"),
        fps = await glob("**", staticPath, cfg.configuration.ignorePatterns);
      return (
        await fs3.promises.cp(staticPath, joinSegments(argv.output, "static"), {
          recursive: !0,
        }),
        fps.map((fp) => joinSegments(argv.output, "static", fp))
      );
    },
  }),
  "Static",
);
var spa_inline_default = `// node_modules/micromorph/dist/index.js
...
/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */`;
var DEFAULT_SANS_SERIF =
    '-apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif',
  DEFAULT_MONO = "ui-monospace, SFMono-Regular, SF Mono, Menlo, monospace";
function googleFontHref(theme) {
  let { code, header, body } = theme.typography;
...
function getComponentResources(ctx) {
  ...
  for (let emitter of ctx.cfg.plugins.emitters) {
    let components = emitter.getQuartzComponents(ctx);
    for (let component of components) allComponents.add(component);
  }
  ...
  for (let component of allComponents) {
    let { css, beforeDOMLoaded, afterDOMLoaded } = component;
    css && componentResources.css.add(css),
      beforeDOMLoaded &&
        componentResources.beforeDOMLoaded.add(beforeDOMLoaded),
      afterDOMLoaded && componentResources.afterDOMLoaded.add(afterDOMLoaded);
  }
  return {
...
function addGlobalPageResources(ctx, staticResources, componentResources) {
  ...
  if (
    ...
    cfg.analytics?.provider === "plausible" &&
...
var defaultOptions11 = { fontOrigin: "googleFonts" },
  ComponentResources = __name((opts) => {
    let { fontOrigin } = { ...defaultOptions11, ...opts };
    return {
      ...
      async emit(ctx, _content, resources, emit) {
        let componentResources = getComponentResources(ctx);
        fontOrigin === "googleFonts" &&
          resources.css.push(googleFontHref(ctx.cfg.configuration.theme)),
          addGlobalPageResources(ctx, resources, componentResources);
        let stylesheet = joinStyles(
            ctx.cfg.configuration.theme,
            base_default,
            ...componentResources.css,
          ),
          prescript = joinScripts(componentResources.beforeDOMLoaded),
          postscript = joinScripts(componentResources.afterDOMLoaded);
        return await Promise.all([
          ...
          emit({ slug: "prescript", ext: ".js", content: prescript }),
          emit({ slug: "postscript", ext: ".js", content: postscript }),
        ]);
      },
    };
  }, "ComponentResources");
function getStaticResourcesFromPlugins(ctx) {
  let staticResources = { css: [], js: [] };
  for (let transformer of ctx.cfg.plugins.transformers) {
    let res = transformer.externalResources
      ? transformer.externalResources(ctx)
      : {};
    res?.js && staticResources.js.push(...res.js),
      res?.css && staticResources.css.push(...res.css);
  }
  return staticResources;
}
__name(getStaticResourcesFromPlugins, "getStaticResourcesFromPlugins");
async function emitContent(ctx, content) {
  let { argv, cfg } = ctx,
    perf = new PerfTimer(),
    log = new QuartzLogger(ctx.argv.verbose);
  log.start("Emitting output files");
  let emit = __name(async ({ slug: slug2, ext, content: content2 }) => {
      let pathToPage = joinSegments(argv.output, slug2 + ext),
        dir = path11.dirname(pathToPage);
      return (
        await fs4.promises.mkdir(dir, { recursive: !0 }),
        await fs4.promises.writeFile(pathToPage, content2),
        pathToPage
      );
    }, "emit"),
    emittedFiles = 0,
    staticResources = getStaticResourcesFromPlugins(ctx);
  for (let emitter of cfg.plugins.emitters)
    try {
      let emitted = await emitter.emit(ctx, content, staticResources, emit);
      if (((emittedFiles += emitted.length), ctx.argv.verbose))
        for (let file of emitted) console.log(`[emit:${emitter.name}] ${file}`);
    } catch (err) {
      trace(`Failed to emit from plugin \`${emitter.name}\``, err);
    }
  log.end(
    `Emitted ${emittedFiles} files to \`${
      argv.output
    }\` in ${perf.timeSince()}`,
  );
}
__name(emitContent, "emitContent");
var config = {
    configuration: {
    ...
    plugins: {
      transformers: [
        ...
        SyntaxHighlighting(),
        ObsidianFlavoredMarkdown({ enableInHtmlEmbed: !1 }),
        GitHubFlavoredMarkdown(),
      ...
      emitters: [
        ...
        FolderPage(),
        TagPage(),
        ContentIndex({ enableSiteMap: !0, enableRSS: !0 }),
        Assets(),
        Static(),
      ],
    },
  },
  quartz_config_default = config;
...
var options = {
  retrieveSourceMap(source) {
    if (source.includes(".quartz-cache")) {
      ...
      return { map: fs5.readFileSync(realSource, "utf8") };
    } else return null;
  },
};
sourceMapSupport.install(options);
async function buildQuartz(argv, mut, clientRefresh) {
  let ctx = { argv, cfg: quartz_config_default, allSlugs: [] },
    perf = new PerfTimer(),
    output = argv.output,
    pluginCount = Object.values(quartz_config_default.plugins).flat().length,
    pluginNames = __name(
      (key) => quartz_config_default.plugins[key].map((plugin) => plugin.name),
      "pluginNames",
    );
  argv.verbose &&
  ...
  let allFiles = await glob(
      "**/*.*",
      argv.directory,
      quartz_config_default.configuration.ignorePatterns,
    ),
    fps = allFiles.filter((fp) => fp.endsWith(".md")).sort();
  console.log(
    `Found ${fps.length} input files from \`${
      argv.directory
    }\` in ${perf.timeSince("glob")}`,
  );
  let filePaths = fps.map((fp) => joinSegments(argv.directory, fp));
  ctx.allSlugs = allFiles.map((fp) => slugifyFilePath(fp));
  let parsedFiles = await parseMarkdown(ctx, filePaths),
    filteredContent = filterContent(ctx, parsedFiles);
  if (
...
__name(buildQuartz, "buildQuartz");
async function startServing(ctx, mut, initialContent, clientRefresh) {
  let { argv } = ctx,
    ignored = await isGitIgnored(),
    contentMap = new Map();
  for (let content of initialContent) {
    let [_tree, vfile] = content;
    contentMap.set(vfile.data.filePath, content);
  }
  let initialSlugs = ctx.allSlugs,
    lastBuildMs = 0,
    toRebuild = new Set(),
    toRemove = new Set(),
    trackedAssets = new Set();
  async function rebuild(fp, action) {
    if (ignored(fp)) return;
    fp = toPosixPath(fp);
    let filePath = joinSegments(argv.directory, fp);
    if (path12.extname(fp) !== ".md") {
      action === "add" || action === "change"
        ? trackedAssets.add(filePath)
        : action === "delete" && trackedAssets.delete(filePath),
        clientRefresh();
      return;
    }
    action === "add" || action === "change"
      ? toRebuild.add(filePath)
      : action === "delete" && toRemove.add(filePath);
    ...
    let perf = new PerfTimer();
    console.log(chalk3.yellow("Detected change, rebuilding..."));
    try {
      let filesToRebuild = [...toRebuild].filter((fp2) => !toRemove.has(fp2)),
        trackedSlugs = [
          ...new Set([...contentMap.keys(), ...toRebuild, ...trackedAssets]),
        ]
          .filter((fp2) => !toRemove.has(fp2))
          .map((fp2) =>
            slugifyFilePath(path12.posix.relative(argv.directory, fp2)),
          );
      ctx.allSlugs = [...new Set([...initialSlugs, ...trackedSlugs])];
      let parsedContent = await parseMarkdown(ctx, filesToRebuild);
      for (let content of parsedContent) {
        let [_tree, vfile] = content;
        contentMap.set(vfile.data.filePath, content);
      }
      for (let fp2 of toRemove) contentMap.delete(fp2);
      let parsedFiles = [...contentMap.values()],
        filteredContent = filterContent(ctx, parsedFiles);
      await rimraf(argv.output),
        await emitContent(ctx, filteredContent),
        console.log(chalk3.green(`Done rebuilding in ${perf.timeSince()}`));
    } catch {
      console.log(
        chalk3.yellow(
    ...
    clientRefresh(), toRebuild.clear(), toRemove.clear(), release();
  }
...
__name(startServing, "startServing");
var build_default = __name(async (argv, mut, clientRefresh) => {
  try {
